<template><div><h1 id="memo-provider节点命令详解" tabindex="-1"><a class="header-anchor" href="#memo-provider节点命令详解" aria-hidden="true">#</a> MEMO provider节点命令详解</h1>
<h1 id="前言" tabindex="-1"><a class="header-anchor" href="#前言" aria-hidden="true">#</a> 前言</h1>
<p>本文将详细介绍MEMO系统provider节点的命令行功能使用；用户需要首先下载二进制并执行程序 <strong>mefs-provider</strong> ，从而启动Memo provider节点。</p>
<h2 id="简介" tabindex="-1"><a class="header-anchor" href="#简介" aria-hidden="true">#</a> 简介</h2>
<p>查看provider节点中可以使用的所有功能简介和用法</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>示例设置节点根目录为：~/.memo-provider</p>
...
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>mefs-provider wallet <span class="token builtin class-name">export</span> <span class="token variable">$wallet</span>
...
<p>配置文件所在路径：节点根目录下 &quot;.memo-provider/config.json&quot;</p>
...
<p>NAME:
mefs-provider config set - Set config key</p>
<p>USAGE:
mefs-provider config set [command options] [arguments...]</p>
<p>OPTIONS:
...
<p>此命令首先为钱包地址注册一个MEMO系统的账户ID，然后将该钱包账户注册为指定角色（provider、Keeper、provider），最后将该角色加入指定的group中；</p>
<p>请注意：provider节点调用<code v-pre>daemon start</code>启动过程中会自动根据参数指定生成账户id、注册角色、加入group；</p>
...
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>NAME:
   mefs-provider register - register an account <span class="token function">id</span> <span class="token keyword">for</span> the wallet first, <span class="token keyword">then</span> register a role <span class="token keyword">for</span> it, at last, <span class="token function">add</span> it into a group.

USAGE:
   mefs-provider register <span class="token punctuation">[</span>command options<span class="token punctuation">]</span> <span class="token punctuation">[</span>role <span class="token punctuation">(</span>provider/keeper/provider<span class="token punctuation">)</span><span class="token punctuation">]</span> <span class="token punctuation">[</span>group index<span class="token punctuation">]</span>
...
--help, -h 查看更多帮助</p>
<p>命令示例</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>mefs-provider register <span class="token parameter variable">--pwd</span><span class="token operator">=</span>memoriae provider <span class="token number">1</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>参数说明：pwd为钱包密码，provider表示指定的角色为provider，1为指定此节点加入group 1</p>
...
.xx.xx.xx/xxx.xxx.xxx.xxx/tcp/xxx/p2p/12D3K...</p>
...
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>mefs-provider net connect /ip4/1.2.3.4/tcp/5678/p2p/12D<span class="token punctuation">..</span>
...
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>mefs-provider net findpeer 12D3KooWHWKAGipKn9QCBEnNxf1ERhRud9qPsWTu9PqFrBm9A9kz
...
<p>用于provider节点申明自己的公网ip地址；(仅provider节点需要使用该命令)</p>
...
<p>此命令使节点退出目前的角色，此时将不再能使用角色相关功能。但钱包余额不受影响，并且对于provider和keeper角色，此时能够取出注册角色时提供的角色质押金额。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="list子命令" tabindex="-1"><a class="header-anchor" href="#list子命令" aria-hidden="true">#</a> list子命令</h3>
<p>列出在此provider节点上存有数据的所有user</p>
...
<p>列出所有与provider节点的订单状态</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>mefs-provider order payList
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h1 id="三-provider日常使用命令" tabindex="-1"><a class="header-anchor" href="#三-provider日常使用命令" aria-hidden="true">#</a> 三：provider日常使用命令</h1>
...
<p>provider节点设置白名单功能，创建桶时，优先选择白名单内节点。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>添加provider节点至白名单</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>mefs-provider restrict add PID
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>参数PID为要添加至白名单的provider节点ID</p>
<p>查看白名单内节点</p>
...
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>mefs-provider restrict has PID
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>参数PID为要添加至白名单的provider节点ID</p>
<p>删除白名单节点</p>
...
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>mefs-provider auth info
...
<p>mefs-provider log setLevel level (debug/info/warn/error)</p>
<p>设置等级实例：</p>
