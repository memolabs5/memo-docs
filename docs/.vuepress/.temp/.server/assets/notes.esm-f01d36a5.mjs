...
function m(t2, e2, n2) {
  ...
  e2 && !g.test(n2) && (n2 = function(t3, e3) {
    ...
    return "//" === e3.substring(0, 2) ? n3 ? e3 : t3.replace(w, "$1") + e3 : "/" === e3.charAt(0) ? n3 ? e3 : t3.replace(x, "$1") + e3 : t3 + e3;
  }(e2, n2));
  try {
...
class O {
  ...
  parse(t2, e2 = true) {
    let n2, i2, s2, r2, a2, l2, o2, c2, u2, d2, h2, g2, m2, f2, k2, w2, x2, b2, y2, v2 = "";
    const S2 = t2.length;
    for (n2 = 0; n2 < S2; n2++)
        ...
        switch (d2.type) {
          ...
          case "list":
            for (h2 = d2.ordered, g2 = d2.start, m2 = d2.loose, r2 = d2.items.length, u2 = "", i2 = 0; i2 < r2; i2++)
              k2 = d2.items[i2], w2 = k2.checked, x2 = k2.task, f2 = "", k2.task && (b2 = this.renderer.checkbox(w2), m2 ? k2.tokens.length > 0 && "paragraph" === k2.tokens[0].type ? (k2.tokens[0].text = b2 + " " + k2.tokens[0].text, k2.tokens[0].tokens && k2.tokens[0].tokens.length > 0 && "text" === k2.tokens[0].tokens[0].type && (k2.tokens[0].tokens[0].text = b2 + " " + k2.tokens[0].tokens[0].text)) : k2.tokens.unshift({ type: "text", text: b2 }) : f2 += b2), f2 += this.parse(k2.tokens, m2), u2 += this.renderer.listitem(f2, x2, w2);
