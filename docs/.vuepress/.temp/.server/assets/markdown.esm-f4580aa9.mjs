...
class T {
  ...
  link(e2) {
    ...
    if (t2) {
      ...
      return n2 = n2.trim(), /^</.test(n2) && (n2 = this.options.pedantic && !/>$/.test(e3) ? n2.slice(1) : n2.slice(1, -1)), A(t2, { href: n2 ? n2.replace(this.rules.inline._escapes, "$1") : n2, title: r2 ? r2.replace(this.rules.inline._escapes, "$1") : r2 }, t2[0], this.lexer);
    }
  }
  reflink(e2, t2) {
    let n2;
    if ((n2 = this.rules.inline.reflink.exec(e2)) || (n2 = this.rules.inline.nolink.exec(e2))) {
      ...
      return A(n2, e3, n2[0], this.lexer);
    }
  }
  emStrong(e2, t2, n2 = "") {
...
class q {
  constructor(e2) {
    this.tokens = [], this.tokens.links = /* @__PURE__ */ Object.create(null), this.options = e2 || t, this.options.tokenizer = this.options.tokenizer || new T(), this.tokenizer = this.options.tokenizer, this.tokenizer.options = this.options, this.tokenizer.lexer = this, this.inlineQueue = [], this.state = { inLink: false, inRawBlock: false, top: true };
...
class O {
  constructor(e2) {
    this.options = e2 || t, this.options.renderer = this.options.renderer || new L(), this.renderer = this.options.renderer, this.renderer.options = this.options, this.textRenderer = new Z(), this.slugger = new C();
  }
  static parse(e2, t2) {
...
N.options = N.setOptions = function(e2) {
  ...
  e2.forEach((e3) => {
    ...
    if (e3.tokenizer) {
      const n3 = N.defaults.tokenizer || new T();
...
}, N.Parser = O, N.parser = O.parse, N.Renderer = L, N.TextRenderer = Z, N.Lexer = q, N.lexer = q.lex, N.Tokenizer = T, N.Slugger = C, N.parse = N;
/*!
 * The reveal.js markdown plugin. Handles parsing of
 * markdown inside of presentations as well as loading
 * of external markdown documents.
 */
const D = "__SCRIPT_END__", P = /\[([\s\d,|-]*)\]/, M = { "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" };
const markdown_esm = () => {
  let e2;
  function t2(e3) {
    var t3 = (e3.querySelector("[data-template]") || e3.querySelector("script") || e3).textContent, n3 = (t3 = t3.replace(new RegExp(D, "g"), "<\/script>")).match(/^\n?(\s*)/)[1].length, r3 = t3.match(/^\n?(\t*)/)[1].length;
  ...
  function s2(e3, t3) {
    ...
    return 2 === n3.length && (e3 = n3[0] + '<aside class="notes">' + N(n3[1].trim()) + "</aside>"), '<script type="text/template">' + (e3 = e3.replace(/<\/script>/g, D)) + "<\/script>";
  }
  function i2(e3, t3) {
    t3 = r2(t3);
  ...
  return { id: "markdown", init: function(t3) {
    ...
    return n3 || (n3 = new N.Renderer(), n3.code = (e3, t4) => {
      let n4 = "";
      return P.test(t4) && (n4 = t4.match(P)[1].trim(), n4 = `data-line-numbers="${n4}"`, t4 = t4.replace(P, "").trim()), `<pre><code ${n4} class="${t4}">${e3 = e3.replace(/([&<>'"])/g, (e4) => M[e4])}</code></pre>`;
